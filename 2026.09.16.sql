...
-- 1. EMP 테이블에서 학생들의 사원의 이름을 첫글자는 대문자로, 나머지는 소문자로 출력하시오.
select INITCAP(ename)
from emp;

-- 2. student 테이블에서 학생들의 이름을 모두 대문자, 모두 소문자로 출력하시오.
select UPPER(ename) as "대문자", LOWER(ename) as "소문자"
from emp;

-- 3. 학생들의 이름과 이름의 길이, 바이트수를 출력하시오.
select ename, LENGTH(ename) as "길이", LENGTHB(ename) as "바이트수"
from emp;

-- 4. 교수(professor)테이블에서 교수의 name과 position을 다음과 같이 출력하시오.
--   *단, concat 함수 사용할 것.*
--    profno   이름및직급
-- 예) 1001   조인형-정교수
select profno, concat(name, concat('-', position)) as "이름및직급"
from professor;

-- 5. 교수들의 이름과 입사년도를 출력하시오.
select name, SUBSTR(hiredate, 1, 4)
from professor;

-- 6. 사원테이블에서 이름의 글자수가 6자 이상인 사람의 이름을 앞에서 3자만 구하여 첫글자만 대문자로 출력하고 나머지는 소문자 출력하시오.
select ename, INITCAP(SUBSTR(ename, 1, 3)) as "답"
from emp
where LENGTH(ename) >= 6;

-- 7. Student 테이블을 참조해서 1 전공이 201번인 학생의 이름과 전화번호와 지역번호를 출력하세요.
select name, tel, SUBSTR(tel, 1, INSTR(tel, ')', 1, 1) - 1) as "지역번호"
from Student
where deptno1 = 201;

-- 8. emp 테이블에서 deptno가 10번인 사원들의 이름을 총 9자리로 출력하되 오른쪽 빈자리에는 해당 자리 수에 해당되는 숫자가 출력되도록하세요.
select RPAD(ename, 9, SUBSTR('123456789', LENGTH(ename) + 1)) as "RPAD"
from emp
where deptno = 10;

-- 9. Student 테이블에서 1 전공이 102번인 학생들의 이름과 전화번호, 전화번호에서 국번 부분만 '*' 처리하여 출력하세요
-- 모든 국번은 3자리로 간주
...
SUBSTR('*****', 1, INSTR(tel, '-', 1, 1) - INSTR(tel, ')', 1, 1) - 1)) as "REPLACE"
from Student
where deptno1 = 102;